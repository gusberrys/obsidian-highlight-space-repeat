...
export class SubjectModal extends Modal {
	...
	constructor(
		...
		onSubmit: ((subject: Subject) => void) | null = null
	) {
		super(app);
	...
	private collectAvailableOptions(): void {
		...
		const settings = this.coreAPI?.getCategories?.() || [];

		// Keywords
		for (const category of (Array.isArray(settings) ? settings : settings?.categories || [])) {
		...
		const codeBlocks = this.coreAPI?.getCodeBlocks?.() || [];
		this.allLanguages = codeBlocks.map((l: any) => l.id).sort();
	...
	private renderChip(
		...
		if (type === 'keyword') {
			...
			if (isLanguage) {
				// Style as language
				const codeBlocks = this.coreAPI?.getCodeBlocks?.() || [];
				const lang = codeBlocks.find((l: any) => l.id === value);
				icon = lang?.icon || '💻';
			...
			switch (col.type) {
				...
				case 'input':
					...
					input.addEventListener('blur', async () => {
						// updateTopic removed - save happens on modal close
						this.renderMatrixPreview();
					});
					break;
	...
	private renderMatrixSection(containerEl: HTMLElement): void {
		...
		// MatrixRenderer.renderMatrixTable removed
	}
