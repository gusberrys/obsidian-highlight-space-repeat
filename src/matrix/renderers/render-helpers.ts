...
export function renderFHRCountLinkBadges(
	...
	if (cell.shouldShowFiles()) {
		...
		if (fileCount > 0) {
			...
			fileCountSpan.addEventListener('click', (e) => {
				e.stopPropagation();
				console.log('🔴🔴🔴 NEW CODE LOADED - BUILD 00:27:00 🔴🔴🔴');
				console.log('[renderFHRCountLinkBadges] File count clicked:', cellKey);
	...
	if (cell.shouldShowHeaders()) {
		...
		if (headerCount > 0) {
			...
			headerCountSpan.addEventListener('click', (e) => {
				e.stopPropagation();
				console.log('🔴🔴🔴 NEW CODE LOADED - BUILD 00:27:00 🔴🔴🔴');
				console.log('[renderFHRCountLinkBadges] Header count clicked:', cellKey);
	...
	if (recordCount > 0) {
		...
		recordCountSpan.addEventListener('click', (e) => {
			e.stopPropagation();
			console.log('🔴🔴🔴 NEW CODE LOADED - BUILD 00:27:00 🔴🔴🔴');
			console.log('[renderFHRCountLinkBadges] Record count clicked:', cellKey);
