...
export class MatrixRenderer {
	...
	private renderSubjectHeaderCell(headerRow: HTMLTableRowElement): void {
		...
		const iconBtn = selectorDiv.createEl('button', {
			...
			title: `Click: Show subject columns\nCmd/Ctrl/Alt+Click: Edit subject settings\n\nSubject: ${this.subject.name}`
		});
	...
	private renderSecondaryHeaderCell(headerRow: HTMLTableRowElement, topic: Topic, col: number): void {
		...
		// Add counts and tooltip
		const cellInstance = this.cellInstances.get(cellKey);
		if (cellInstance) {
			// Get expressions from cell
			const F = cellInstance.getFExpression();
			const H = cellInstance.getHExpression();
			const R = cellInstance.getFilterExpression();

			const expressionLines: string[] = [];
			if (F !== null) expressionLines.push(`F: ${F}`);
			if (H !== null) expressionLines.push(`H: ${H}`);
			if (R !== null) expressionLines.push(`R: ${R}`);
	...
	private renderPrimaryRowHeader(row: HTMLTableRowElement, primaryTopic: Topic, rowNum: number): void {
		...
		// Get cell instance and set tooltip
		const cellInstance = this.cellInstances.get(cellKey);
		...
		if (cellInstance) {
			...
			if (R !== null) expressionLines.push(`R: ${R}`);
			const expressionsText = expressionLines.length > 0 ? '\n\n' + expressionLines.join('\n') : '';
			tooltipText = `${primaryTopic.name}${expressionsText}`;
		...
		rowHeaderCell.setAttribute('title', `Click: Open column | Cmd+Click: Open dashboard\n\n${tooltipText}`);
	...
	private hasLimitedCollection(secondaryTopic: Topic | null, primaryTopic: Topic | null): boolean {
		...
		if (secondaryTopic && primaryTopic) {
			return (secondaryTopic.fhDisabled || false) || (primaryTopic.fhDisabled || false);
		}
		// For single topic: check that topic's flags
		const topic = secondaryTopic || primaryTopic;
		if (topic) {
			return topic.fhDisabled || false;
		}
