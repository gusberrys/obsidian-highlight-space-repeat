...
export abstract class MatrixCell {
	...
	collectRecords(allRecords: ParsedFile[]): Array<{ entry: FlatEntry; file: ParsedFile }> {
		if (this.cachedRecords) {
			console.log(`[MatrixCell.collectRecords] USING CACHED (${this.cachedRecords.length} records)`);
			return this.cachedRecords;
		}

		console.log(`[MatrixCell.collectRecords] CALLING doCollectRecords`);
	...
	countRecords(allRecords: ParsedFile[]): number {
		const records = this.collectRecords(allRecords);
		console.log(`[MatrixCell.countRecords] Returning count: ${records.length}`);
	...
	// F/H expression methods (NEW)
	abstract getFExpression(): string | null;  // F: file filter
	abstract getHExpression(): string | null;  // H: header filter
}
